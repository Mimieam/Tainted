    ...
    return function(orientation) {
        ...
        delay(svg, function(animate) {
            animate(polygon)
                ...
                .attr('points', makeHexagonShape(size, orientation));
...
function makeNeighbors(id_diagram, id_code, converter, parity_var) {
    ...
    function neighbor(odd, i) {
        ...
        var h2 = diagram.converter(base.add(Cube.direction(i)));
        ...
        diagram.onUpdate(function() {
            diagram.tiles.selectAll("text").text(function(d) {
                if (d.key < numSpans) {
                    ...
                    return ["q", h.q > 0? "+" : "", h.q == 0? "" : h.q, ", ",
                            "r", h.r > 0? "+" : "", h.r == 0? "" : h.r].join("");
                } else if (numSpans == 12) {
                    return (d.key == 12)? "EVEN" : "ODD";
                } else {
                    return "axial";
                }
            });
...
function makeDistances() {
    ...
    diagram.tiles
        .each(function(d) {
            var len = d.cube.length();
            var text = d3.select(this);
            text.select(".q").classed('highlight', Math.abs(d.cube.v[0]) == len);
            text.select(".s").classed('highlight', Math.abs(d.cube.v[1]) == len);
            text.select(".r").classed('highlight', Math.abs(d.cube.v[2]) == len);
        });
...
function adjustTextForOrientation(orientation) {
    d3.selectAll(".orientation-index-adjust-degrees")
        .text(orientation? " + 30°" : "");
    d3.selectAll(".orientation-index-adjust")
        .text(orientation? "(i + 0.5)" : "i");
...
function makeSpiral(id, spiral) {
    ...
    var N = 0;
    diagram.tiles
        .on('mouseover', function(d) { N = d.cube.length(); redraw(); })
...
function makeHexRegion() {
    ...
    var xline1 = diagram.root.append('line').attr('class', "x-axis");
    var xline2 = diagram.root.append('line').attr('class', "x-axis");
    var yline1 = diagram.root.append('line').attr('class', "y-axis");
    var yline2 = diagram.root.append('line').attr('class', "y-axis");
    var zline1 = diagram.root.append('line').attr('class', "z-axis");
    var zline2 = diagram.root.append('line').attr('class', "z-axis");
    ...
    function moveLine(line, a, b) {
        a = diagram.grid.hexToCenter(a);
        b = diagram.grid.hexToCenter(b);
        var d = b.subtract(a);
        if (d.length() > 0) d = d.normalize().scale(300);
...
function makeIntersection() {
    ...
    function redraw() {
        var N = 3, x = center.v[0], y = center.v[1], z = center.v[2];
...
function makeMovementRange() {
    ...
    function redraw() {
        var maxMovement = 4;
        var start = new Cube(0, 0, 0);
        var visited = d3.map(); visited.set(start, 0);
        var fringes = [[start]];
        for (var k = 0; k < maxMovement; k++) {
            fringes[k+1] = [];
            fringes[k].forEach(function(cube) {
                for (var dir = 0; dir < 6; dir++) {
                    var neighbor = cube.add(Cube.direction(dir));
                    if (!visited.has(neighbor) && !diagram.selected.has(neighbor)) {
        ...
        diagram.tiles
            ...
            .classed('center', function(d) { return d.cube.v[0] == 0 && d.cube.v[1] == 0 && d.cube.v[2] == 0; });
        diagram.tiles.selectAll("text")
            .text(function(d) { return visited.has(d.cube)? visited.get(d.cube) : ""; });
    }
...
function makeLineDrawer() {
    ...
    var root_centers = root.append('g');
    ...
    var offsets = [Cube.direction(1).scale(0.00001)];
    var start = new Cube(-5, 5, 0);
    var goal = new Cube(0, 0, 0);

    function redraw() {
        var d = goal.subtract(start);
        var N = 1;
        for (var i = 0; i < 3; i++) {
            var distance = Math.abs(d.v[i]);
        ...
        for (var i = 0; i <= N; i++) {
            var hex = start.add(d.scale(i/N));
            var p = diagram.grid.hexToCenter(hex);
            ...
            offsets.forEach(function(offset) {
                var hex2 = hex.add(offset).round();
                var p2 = diagram.grid.hexToCenter(hex2);

                data.push({hex: hex, p: p, hex2: hex2, p2: p2});
                if (!hex2.equals(prev)) {
                    selection.add(hex2);
                    prev = hex2;
                }
        ...
        var path = root_path.append('path')
            ...
            .attr('d', ['M', data[0].p, 'L', data[data.length-1].p].join(" "));

        var exacts = root_exacts.selectAll("circle.exact").data(data, function(d, i) { return i; });
        exacts.exit().remove();
...
function makeFieldOfView() {
    ...
    function redraw() {
        ...
        var center = new Cube(0, 0, 0);
        for (var x = 0; x < N; x++) {
            [new Cube(N-x, x, -N), new Cube(x, -N, N-x), new Cube(-N, N-x, x),
             new Cube(-N+x, -x, N), new Cube(-x, N, -N+x), new Cube(N, -N+x, -x)
            ].forEach(function(corner) {
                 var path = Cube.makeLine(center.add(corner.scale(1e-6)), corner);
                 for (var i = 0; i < path.length; i++) {
                     if (diagram.selected.has(path[i])) { break; }
                     visible.add(path[i]);
                 }
            });
        }
...
function makePixelToHex() {
    var diagram = makeGridDiagram(d3.select("#diagram-pixel-to-hex"), Grid.hexagonalShape(2));
    diagram.addCubeCoordinates(false);
...
function makeMapStorage(config, scale) {
    ...
    diagram.nodes.forEach(function(node) {
        var q = node.cube.v[0], r = node.cube.v[2];
    ...
    function highlight(cube) {
        ...
        diagram.tiles.classed('samerow', function(node) { return node.cube.v[2] == cube.v[2]; });
        squares.classed('samerow', function(node) { return node.cube.v[2] == cube.v[2]; });
    }
...
function makeWraparound() {
    ...
    diagram.tiles
        .classed('center', function(d, i) { return shape_mirror[i].length() == 0; })
        .classed('wrapped', function(d) { return d.cube.length() > 2; })
        .classed('parity', function(d, i) { var c = shape_center[i], a = c.length(); return c.v[0] == a || c.v[1] == a || c.v[2] == a; });
...
var neighbors_diagonal = makeGridDiagram(d3.select("#neighbors-diagonal"),
    ...
    .addCubeCoordinates(false);
neighbors_diagonal.tiles.classed('highlight', function(d) { return d.cube.length() == 2; });
...
function orient(orientation) {
    ...
    delay(d3.select("#cube-to-hex"), function(animate) {
        animate(d3.select("#cube-to-hex > g"))
            .attr('transform', "translate(139.5, 139.5) rotate(" + ((!orientation) * 30) + ")");
    });
...
var neighbors_offset = makeNeighbors("#neighbors-offset", "#neighbors-offset-code", Grid.cubeToOddR, "r").update(65, true);
d3.select("#neighbors-offset-odd-r").node().checked = true;
d3.select("#neighbors-offset-odd-r").on('change', function() { neighbors_offset.converter = Grid.cubeToOddR; neighbors_offset.parity_var = "r"; neighbors_offset.update(65, true); });
d3.select("#neighbors-offset-even-r").on('change', function() { neighbors_offset.converter = Grid.cubeToEvenR; neighbors_offset.parity_var = "r"; neighbors_offset.update(65, true); });
d3.select("#neighbors-offset-odd-q").on('change', function() { neighbors_offset.converter = Grid.cubeToOddQ; neighbors_offset.parity_var = "q"; neighbors_offset.update(65, false); });
d3.select("#neighbors-offset-even-q").on('change', function() { neighbors_offset.converter = Grid.cubeToEvenQ; neighbors_offset.parity_var = "q"; neighbors_offset.update(65, false); });
